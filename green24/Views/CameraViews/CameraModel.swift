...
class CameraModel: NSObject, ObservableObject, AVCapturePhotoCaptureDelegate, AVCaptureFileOutputRecordingDelegate {
    ...
    @Published var wasVideo: Bool = false
    ...
    func saveVideo(){
        let video = AVMovie(data: self.mediaData)
        
        UISaveVideoAtPathToSavedPhotosAlbum(previewURL!.path, nil, nil, nil)
        
        self.isSaved = true
    }
    
    func savePic(){
        let image = UIImage(data: self.mediaData)!
